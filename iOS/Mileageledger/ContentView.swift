...
struct ContentView: View {
    ...
    @State private var editingEntry: MileageledgerEntry?
    ...
    var body: some View {
        NavigationStack {
                        ...
                        ForEach(store.entries) { entry in
                            Button {
                                ...
                                VStack(alignment: .leading, spacing: 4) {
                                    ...
                                    if !entry.odometer.isEmpty {
                                        Text("Odometer: \(entry.odometer)")
                                            .font(.caption)
                                            .foregroundStyle(Theme.accent)
                                    }
                                }
            ...
            .navigationTitle("Mileage Ledger")
            .toolbar {
...
enum EntryFormMode {
    case add
    case edit(MileageledgerEntry)
}

struct EntryFormView: View {
    let mode: EntryFormMode
    var onSave: (MileageledgerEntry) -> Void
        ...
        NavigationStack {
            Form {
                Section {
                    ...
                    TextField("Odometer", text: $field2)
                        .accessibilityIdentifier("field2Field")
                        .focused($focusedField, equals: .field2)
                    TextField("Purpose", text: $field3)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        var entry: MileageledgerEntry
                        if case .edit(let existing) = mode {
                            entry = existing
                        } else {
                            entry = MileageledgerEntry(title: "")
                        }
                        entry.title = title.isEmpty ? "Untitled" : title
                        entry.date = date
                        entry.odometer = field2
                        entry.purpose = field3
                        entry.note = note
                        onSave(entry)
                        dismiss()
                    }
            ...
            .onAppear {
                if case .edit(let existing) = mode {
                    ...
                    date = existing.date
                    field2 = existing.odometer
                    field3 = existing.purpose
                    note = existing.note
                }
            }
        }
